...
Translator* new_translator(){
    ...
    Translator* trans = malloc(sizeof(Translator));
    trans->tree = root;
    trans->current = root;
    trans->space_count = 0;
    return trans;
...
void translate(Translator* translator, int c){
    if (c == ' ') {
        translator->space_count += 1;
        if (translator->space_count == 1) {
            printf("%c", translator->current->label);
            translator->current = translator->tree;
        } else if (translator->space_count == 2) {
            printf(" ");
            translator->space_count = 0;
        }
    } else if (c == '.') {
        translator->current = translator->current->dot;
        if (translator->space_count == 1) {
            translator->space_count = 0;
        }
    } else if (c == '-') {
        translator->current = translator->current->dash;
        if (translator->space_count == 1) {
            translator->space_count = 0;
        }
    }
